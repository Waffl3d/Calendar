...
export default function App() {
  ...
  const toggleDone = (index) => {
    ...
    if (!rem.done) {
      const now = new Date();
      const extra = Math.max(0, Math.floor((now - new Date(rem.reminderTime)) / 60000));
      rem.overdueOffset = extra;
      rem.lastCheckTime = now;
    } else {
      const now = new Date();
      if (rem.lastCheckTime) {
        const pausedDuration = Math.floor((now - rem.lastCheckTime) / 60000);
        rem.overdueOffset += pausedDuration;
      }
